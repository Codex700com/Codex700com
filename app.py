...
def add_notif(phone, title, body=""):
    if phone in users:
        ...
        users[phone]['notif'].insert(0, {'title':title,'body':body,'time':dtn.now().strftime("%Y-%m-%d %H:%M")})
...
def notifications2():
    ...
    u=get_user()
    items=""
    for n in u.get('notif',[])[:30]:
        items+="<div class='card'><b>"+n['title']+"</b><br><small>"+n['time']+"</small><p>"+n['body']+"</p></div>"
...
def admin_notify():
    ...
    if request.method=='POST':
        ...
        if title:
            targets = list(users.keys()) if target=='all' else [target]
            for ph in targets: add_notif(ph,title,body)
            msg="Sent to "+target
