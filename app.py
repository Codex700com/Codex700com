...
def init():
 c=db()
 c.executescript("CREATE TABLE IF NOT EXISTS users(id INTEGER PRIMARY KEY,name TEXT,phone TEXT UNIQUE,password TEXT,invite TEXT,balance INTEGER DEFAULT 0,refcode TEXT);CREATE TABLE IF NOT EXISTS investments(id INTEGER PRIMARY KEY,user_id INT,plan TEXT,amount INT,active INT DEFAULT 1,date TEXT);CREATE TABLE IF NOT EXISTS transactions(id INTEGER PRIMARY KEY,user_id INT,type TEXT,amount INT,status TEXT,date TEXT,ref TEXT);CREATE TABLE IF NOT EXISTS checkins(user_id INT,date TEXT PRIMARY KEY);CREATE TABLE IF NOT EXISTS notifications(id INTEGER PRIMARY KEY,user_id INT,msg TEXT,date TEXT);CREATE TABLE IF NOT EXISTS chats(id INTEGER PRIMARY KEY,user_id INT,who TEXT,msg TEXT,date TEXT);CREATE TABLE IF NOT EXISTS messages(id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER,message TEXT,admin_reply TEXT,image TEXT,created_at TEXT);")
 c.commit();c.close()
init()
def need(f):
 def w(*a,**kw):
 ...
 *{box-sizing:border-box}body{margin:0;background:#000000;color:#fff;font-family:Inter,system-ui,Arial;padding-bottom:110px}
 ...
 .logo{text-align:center;line-height:1}.logo b{font-size:32px;color:;letter-spacing:2px}.logo span{font-size:10px;letter-spacing:4px;color:#fff}
 .hero{margin:8px 10px;border:2px solid ;border-radius:16px;padding:16px;background:#111,#111;position:relative;overflow:hidden;box-shadow:0 0 20px 55}
 .hero h4{margin:0;font-weight:600;color:#dbeafe}.hero h2{margin:4px 0;color:#00cfff;font-size:28px}.hero p{margin:6px 0;color:#cbd5e1}.hero i{color:}
 ...
 .banner{margin:10px;border:1.5px solid ;border-radius:14px;padding:12px;display:flex;align-items:center;justify-content:space-between;background:#111}
 ...
 <div class="banner"><div>🏆 <b style="color:">RAFFLE DRAW</b><br><small>More deposits = More chances = Bigger prizes!</small></div><a class="btn-blue" href="/raffle">🎁 VIEW PRIZES →</a></div>
...
<span style="color:#4caf50;font-weight:bold">Drink water, save lives</div>
    <div style="background:#111; border:1px solid #00ff99; border-radius:12px; padding:14px; margin:10px;">
...
if(found){{
 ...
 `<div class="row"><span>VIP requiredred</span><b>VIP0</b></div>`;
...
def checkin_page():
    ...
    for i,amt in enumerate([500,700,1000,1500,2000,3000,5000],1):
        if (claimed and i<=cur_day) or (not claimed and i<cur_day):
            ...
            st='<div style="background:#334155;color:#cbd5e1;font-size:11px;padding:4px 8px;border-radius:12px;margin-top:6px">Pending</div>'; bo="border:1px solid 55"; bg="background:#0a1440"
    ...
    .banner{{margin:10px;border-radius:18px;padding:20px;background:#111;border:1px solid 55;position:relative;overflow:hidden}}
    .card{{margin:10px;border:1px solid 44;border-radius:14px;padding:12px;background:#041338}}
    .bottom{{position:fixed;bottom:0;left:0;right:0;display:flex;background:#000000;border-top:1px solid 33;padding:8px}}
    ...
    <div class="banner"><div style="font-size:38px;font-weight:900;line-height:1">DAILY<br><span style="color:">CHECK-IN</span></div>
    ...
    <div style="text-align:center">🔥<br><b style="color:">{disp_streak} Day</b><br><small>Streak</small></div></div>
    <div class="card"><div style="display:flex;justify-content:space-between;align-items:center"><b>🎁 7-DAY REWARDS</b><span style="font-size:11px;border:1px solid ;padding:4px 8px;border-radius:12px">🔥 Keep streak!</span></div>
