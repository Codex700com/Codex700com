...
def invest_confirm_v2():
    ...
    try:
        ...
        cur.execute("""
            INSERT INTO transactions
            (user_id,type,amount,desc,created_ts)
            VALUES (?,?,?,?,?)
        """, (
            uid,
            'invest',
...
@app.route('/', endpoint='final_home_new_12')
def new_home_override():
    from flask import session
    u = session.get('user', {})
    name = str(u.get('name','Tamale Imran')).upper()
    # NEW ACCOUNT = ZERO
    wallet = u.get('balance',0)
    invested = u.get('invested',0)
    wallet_f = f"{int(wallet):,}" if isinstance(wallet,(int,float)) else "0"
    invested_f = f"{int(invested):,}" if isinstance(invested,(int,float)) else "0"
    return f"""
<!DOCTYPE html>
<html>
<head><meta name="viewport" content="width=device-width, initial-scale=1">
<style>
body{{margin:0;background:#000;color:#fff;font-family:sans-serif;padding-bottom:80px;}}
.header{{display:flex;justify-content:space-between;align-items:center;padding:12px 15px;background:#000;}}
.logo{{color:#00CCFF;font-size:32px;font-weight:900;letter-spacing:2px;text-align:center;line-height:1;}}
.logo small{{display:block;font-size:10px;letter-spacing:4px;color:#fff;font-weight:400;}}
.welcome{{margin:10px;border:2px solid #00CCFF;border-radius:20px;padding:18px;background:radial-gradient(circle at top, #0a2a4a, #000);position:relative;box-shadow:0 0 15px rgba(0,204,255,0.4);}}
.welcome h1{{color:#00CCFF;font-size:28px;margin:5px 0;font-weight:900;}}
.circle{{position:absolute;right:15px;top:20px;width:110px;height:110px;background:rgba(0,0,0,0.6);border-radius:50%;display:flex;align-items:center;justify-content:center;text-align:center;color:#00CCFF;font-size:13px;font-weight:bold;line-height:1.2;}}
.btn-invest{{background:linear-gradient(180deg,#e0f7ff,#00CCFF);color:#000;border:none;padding:14px 30px;border-radius:30px;font-weight:900;font-size:16px;margin-top:15px;display:inline-block;}}
.stats{{display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px;margin:10px;}}
.card{{background:linear-gradient(180deg,#ffffff,#7dd8ff);border-radius:15px;padding:12px 5px;text-align:center;color:#000;}}
.card b{{display:block;font-size:13px;}} .card span{{color:#004a8f;font-weight:900;font-size:15px;}}
.daily{{margin:10px;background:linear-gradient(90deg,#001a33,#003366);border:2px solid #00CCFF;border-radius:15px;padding:12px;display:flex;justify-content:space-between;align-items:center;}}
.check-btn{{background:#00CCFF;color:#fff;border:none;padding:10px 20px;border-radius:25px;font-weight:bold;}}
.grid{{display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px;margin:10px;}}
.tile{{background:linear-gradient(180deg,#ffffff,#7dd8ff);border-radius:15px;padding:14px 5px;text-align:center;color:#000;}}
.tile.orange{{background:linear-gradient(180deg,#ff9a3d,#ff5a00);color:#fff;}}
.tile b{{display:block;font-size:14px;margin:5px 0;}} .tile small{{font-size:11px;color:#003366;}} .tile.orange small{{color:#fff;}}
.bottom{{position:fixed;bottom:0;left:0;right:0;background:#000;display:flex;justify-content:space-around;padding:10px 0;border-top:1px solid #222;}}
.bottom a{{color:#888;text-align:center;text-decoration:none;font-size:11px;}} .bottom a.active{{color:#00CCFF;}}
</style>
</head>
<body>
<div class="header"><div style="color:#00CCFF;font-size:28px;">☰</div><div class="logo">CODEX<small>INVEST • GROW • WIN</small></div><div>🔔 👤</div></div>
<div class="welcome"><div class="circle">INVEST<br>TODAY<br>BUILD<br>TOMORROW</div><div style="font-size:14px;">👑 WELCOME BACK,</div><h1>{name}</h1><div>Big dreams require action.<br><i style="color:#00CCFF;">You're one step closer to freedom!</i></div><button class="btn-invest" onclick="location.href='/invest'">↗ INVEST NOW →</button></div>
<div class="stats"><div class="card"><div>💼</div><b>Wallet</b><span>UGX<br>{wallet_f}</span></div><div class="card"><div>🪙</div><b>Invested</b><span>UGX<br>{invested_f}</span></div><div class="card"><div>💰</div><b>Income</b><span>UGX 0</span></div><div class="card"><div>📈</div><b>Active</b><span>0</span></div></div>
<div class="daily"><div><div style="color:#00CCFF;font-weight:bold;">🎁 Daily Check-In</div><small>Log in daily and win rewards!</small></div><button class="check-btn" onclick="alert('Checked In! +0 UGX - Deposit first')">📅 CHECK IN →</button></div>
<div class="grid"><div class="tile" onclick="location.href='/invest'"><div>📈</div><b>Invest</b><small>Start your journey</small></div><div class="tile" onclick="location.href='/deposit'"><div>💲</div><b>Deposit</b><small>Fund your wallet</small></div><div class="tile orange" onclick="location.href='/withdraw'"><div>💼</div><b>Withdraw</b><small>Get your earnings</small></div><div class="tile" onclick="location.href='/referrals'"><div>👥</div><b>Referral</b><small>Earn together</small></div></div>
<div class="grid"><div class="tile" onclick="location.href='/transactions'"><div>🧾</div><b>Transactions</b><small>View all records</small></div><div class="tile" onclick="location.href='/raffle'"><div>🎁</div><b>Raffle</b><small>Win amazing prizes</small></div><div class="tile" onclick="location.href='/support'"><div>🎧</div><b>Support</b><small>We are here to help</small></div><div class="tile" onclick="location.href='/chat'"><div>💬</div><b>Chat</b><small>Talk to manager</small></div></div>
<div class="bottom"><a href="/" class="active">🏠<br>HOME</a><a href="/invest">📊<br>INVEST</a><a href="/transactions">⇄<br>TRANSACTIONS</a><a href="/referrals">👥<br>REFERRALS</a><a href="/account">👤<br>ACCOUNT</a></div>
</body></html>
    """
...
# ===== FORCE NEW HOME - ZERO BALANCE - FINAL OVERRIDE =====
@app.route('/force-home-zero', endpoint='force_home_zero_final')
@app.route('/', endpoint='final_zero_home_999')
@app.route('/home', endpoint='final_zero_home_999_b')
def final_zero_home_999():
    from flask import session
    u = session.get('user', {})
    name = str(u.get('name','Tamale Imran')).upper()
    return """
...
.logo{color:#00CCFF;font-size:32px;font-weight:900;letter-spacing:2px;text-align:center;line-height:1}
.logo small{display:block;font-size:10px;letter-spacing:4px;color:#fff;font-weight:400}
.welcome{margin:10px;border:2px solid #00CCFF;border-radius:20px;padding:18px;background:#001122;position:relative}
.welcome h1{color:#00CCFF;font-size:28px;margin:5px 0;font-weight:900}
.circle{position:absolute;right:15px;top:20px;width:110px;height:110px;background:#000;border-radius:50%;display:flex;align-items:center;justify-content:center;text-align:center;color:#00CCFF;font-size:12px;font-weight:bold}
.btn-invest{background:#00CCFF;color:#000;border:none;padding:14px 30px;border-radius:30px;font-weight:900;font-size:16px;margin-top:15px}
.stats{display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px;margin:10px}
.card{background:linear-gradient(180deg,#fff,#7dd8ff);border-radius:15px;padding:12px 5px;text-align:center;color:#000}
.grid{display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:8px;margin:10px}
.tile{background:linear-gradient(180deg,#fff,#7dd8ff);border-radius:15px;padding:14px 5px;text-align:center;color:#000}
.tile.orange{background:linear-gradient(180deg,#ff9a3d,#ff5a00);color:#fff}
.bottom{position:fixed;bottom:0;left:0;right:0;background:#000;display:flex;justify-content:space-around;padding:10px 0;border-top:1px solid #222}
.bottom a{color:#888;text-align:center;text-decoration:none;font-size:11px}
.bottom a.active{color:#00CCFF}
</style>
</head>
<body>
<div class="header"><div style="color:#00CCFF">☰</div><div class="logo">CODEX<small>INVEST • GROW • WIN</small></div><div>🔔 👤</div></div>
<div class="welcome"><div class="circle">INVEST<br>TODAY<br>BUILD<br>TOMORROW</div><div>👑 WELCOME BACK,</div><h1>""" + name + """</h1><div>Big dreams require action.<br><i style="color:#00CCFF;">You're one step closer to freedom!</i></div><button class="btn-invest" onclick="location.href='/invest'">↗ INVEST NOW →</button></div>
<div class="stats"><div class="card">💼<br><b>Wallet</b><br><b style="color:#004a8f">UGX 0</b></div><div class="card">🪙<br><b>Invested</b><br><b style="color:#004a8f">UGX 0</b></div><div class="card">💰<br><b>Income</b><br><b style="color:#004a8f">UGX 0</b></div><div class="card">📈<br><b>Active</b><br><b style="color:#004a8f">0</b></div></div>
<div style="margin:10px;background:#001a33;border:2px solid #00CCFF;border-radius:15px;padding:12px;display:flex;justify-content:space-between;align-items:center"><div><div style="color:#00CCFF;font-weight:bold">🎁 Daily Check-In</div><small>Log in daily and win rewards!</small></div><button style="background:#00CCFF;color:#fff;border:none;padding:10px 20px;border-radius:25px;font-weight:bold" onclick="alert('Checked In!')">📅 CHECK IN →</button></div>
<div class="grid"><div class="tile" onclick="location.href='/invest'">📈<br><b>Invest</b><br><small>Start journey</small></div><div class="tile" onclick="location.href='/deposit'">💲<br><b>Deposit</b><br><small>Fund wallet</small></div><div class="tile orange" onclick="location.href='/withdraw'">💼<br><b>Withdraw</b><br><small>Get earnings</small></div><div class="tile" onclick="location.href='/referrals'">👥<br><b>Referral</b><br><small>Earn together</small></div></div>
<div class="grid"><div class="tile" onclick="location.href='/transactions'">🧾<br><b>Transactions</b><br><small>View records</small></div><div class="tile" onclick="location.href='/raffle'">🎁<br><b>Raffle</b><br><small>Win prizes</small></div><div class="tile" onclick="location.href='/support'">🎧<br><b>Support</b><br><small>We help</small></div><div class="tile" onclick="location.href='/chat'">💬<br><b>Chat</b><br><small>Talk manager</small></div></div>
<div class="bottom"><a href="/" class="active">🏠<br>HOME</a><a href="/invest">📊<br>INVEST</a><a href="/transactions">⇄<br>TRANSACTIONS</a><a href="/referrals">👥<br>REFERRALS</a><a href="/account">👤<br>ACCOUNT</a></div>
</body></html>
    """
